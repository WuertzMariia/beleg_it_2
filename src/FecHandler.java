...
public class FecHandler {
    ...
    public boolean checkCorrection(int nr, HashMap<Integer, RTPpacket> mediaPackets) {
        //TASK complete this method! ob Packet fehlt, dass max 1 Packet fehlt. Ansonsten keinen Sinn
        // return true/false - wenn mehr als 1
        if (fecList.get(nr) == null) return false;

        List<Integer> list = fecList.get(nr);
        Integer count = 0;

        for (Integer lnr : list) {
            if (mediaPackets.get(lnr) != null) count++;
        }
        Integer result = list.size() - count;
        if (result <= 1) {
            return true;
        } else {
            return false;
    ...
    public RTPpacket correctRtp(int nr, HashMap<Integer, RTPpacket> mediaPackets) {
        ...
        // die Nummer des verknüpften FEC-Paket
        int fnrNumber = fecNr.get(nr);

        // Liste von RTP Paketen
        List<Integer> l = fecList.get(nr);

        // get veknüpftes FEC_STACK_PACKET
        fec = fecStack.get(fnrNumber);

        for(Integer lnr: l) {
            if(lnr != nr) {

                //
                fec.addRtp(mediaPackets.get(lnr));
            }
        }
